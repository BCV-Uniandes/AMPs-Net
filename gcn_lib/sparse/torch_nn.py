...
from utils.data_util import get_atom_feature_dims, get_bond_feature_dims, get_aa_feature_dims
...
class AtomEncoder(nn.Module):
    ...
    def __init__(self, emb_dim,args):
        ...
        if args.AA_graph:
            full_atom_feature_dims = get_aa_feature_dims()
        else:
...
class MM_BondEncoder(nn.Module):
    ...
    def forward(self, edge_attr):
        ...
        #bond_embedding = 0
        #for i in range(edge_attr.shape[1]):
        #    edge_attr_i = edge_attr[:, i].clone()
        #    oh = one_hot_vector(edge_attr_i, self.full_bond_feature_dims[i])
        #    oh = ((edge_attr[:, i] + 1) / (edge_attr[:,i] + 1)).unsqueeze(1) * oh
        #    bond_embedding += self.bond_embedding_list[i](oh)
